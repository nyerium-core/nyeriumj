...
import org.nyeriumj.core.listeners.BlockChainListener;
import org.nyeriumj.store.FlatDB;
import org.nyeriumj.store.HashStore;
import org.nyeriumj.store.MasternodeDB;
 ...
 * nyeriumj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating nyeriumj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * @param feePerKb The default fee per 1000 bytes of transaction data to pay when completing transactions. For details, see {@link this.SendRequest#feePerKb}.
     * @param ensureMinRequiredFee Whether to ensure the minimum required fee by default when completing transactions. For details, see {@link this.SendRequest#ensureMinRequiredFee}.
     */
     ...
     * independently use nyeriumj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a nyeriumj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using nyeriumj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing nyeriumj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.nyeriumj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.nyeriumj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.nyeriumj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    // Nyex Specific
    //
    //private boolean initializedNyerium = false;
    public void initNyerium(boolean liteMode, boolean allowInstantX) {
        this.liteMode = true;
        ...
        //initializedNyerium = true;
    }

//    public void closeNyerium() {
//        //Dash Specific
//        sporkManager = null;
//
...
//        //initializedNyerium = false;
//    }

    public void initNyeriumSync(String directory)
    {
        ...
        //other functions
        // todo: furszy
        darkSendPool.startBackgroundProcessing(); //ek uncomment
    }
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (chain!=null) {
            ...
            chain.addListener(this.updateHeadListener);
        }
        //todo: furszy nyex init
        //if (initializedNyerium) {
        if (sporkManager!=null) {
            ...
            log.error("##### Nyerium init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
            instantSend.setBlockChain(chain);
        }
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented nyex MN stuff
    ...
    /**
     * The default fee per 1000 bytes of transaction data to pay when completing transactions. For details, see {@link this.SendRequest#feePerKb}.
     */
    ...
    /**
     * Whether to ensure the minimum required fee by default when completing transactions. For details, see {@link this.SendRequest#ensureMinRequiredFee}.
     */
