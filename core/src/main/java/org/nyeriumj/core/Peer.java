...
import org.nyeriumj.net.StreamConnection;
...
import org.nyeriumj.wallet.Wallet;
 ...
 * {@link org.nyeriumj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.nyeriumj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.nyeriumj.net.NioClientManager#openConnection(java.net.SocketAddress, StreamConnection)}
     * or
     * {@link org.nyeriumj.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.</p>
     *
    ...
    /**
     * <p>Construct a peer that reads/writes from the given block chain. Transactions stored in a {@link org.nyeriumj.core.TxConfidenceTable}
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        // nyeriumj is a client mode implementation. That means there's not much point in us talking to other client
    ...
    protected void processTransaction(final Transaction tx) throws VerificationException {
        ...
        //EK -- try {
        tx.verify();
        //EK -- }catch (VerificationException.CoinbaseScriptSizeOutOfRange e){
        //EK -- log.error("CoinbaseScriptSizeOutOfRange, txId "+tx.getHashAsString(),e.getMessage());
        //EK -- return;
        //EK -- }
        lock.lock();
        try {
            ...
            if (context.instantSend!=null && context.instantSend != null) //EK --  && context.instantSend != null
                ...
                try {
                    if (wallet.isPendingTransactionRelevant(tx)) {
                        if (vDownloadTxDependencyDepth > 0) {
                            ...
                            if(tx instanceof TransactionLockRequest)
							{
                                //EK if (context.instantSend!=null)
                                context.instantSend.acceptLockRequest((TransactionLockRequest)tx);
                            }
                        }
                    }
    ...
    protected ListenableFuture<Object> downloadDependenciesInternal(final int maxDepth, final int depth,
        ...
        try {
            ...
            log.error("{}: Couldn't send getdata in downloadDependencies({})", this, tx.getHash(), e); //ek , e
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    //ek --System.out.println("#### checkForFilterExhaustion true");
...
//            if(!instantSend.mapTxLockVotes.containsKey(item.hash))
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //ek --}else {
                        //ek --    getdata.addFilteredBlock(item.hash);
                        //ek --    pingAfterGetData = true;
                        //ek --    //System.out.println("trying to continue syncing but the block is already on the pending blocks downloads list");
                        }
                    }
                }
        ...
        if (!getdata.getItems().isEmpty()) {
            ...
            sendMessage(getdata);
        //ek --}else {
        //ek --    //System.out.println("for some reason the getData items list is empty..");
        }
     ...
     * Once the pong is received the value returned by {@link org.nyeriumj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.nyeriumj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.nyeriumj.core.Peer#ping()} has never
